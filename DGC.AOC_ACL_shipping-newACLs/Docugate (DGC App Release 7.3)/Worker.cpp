...
int  ResetConnection    ( CHandler &Handler);
...
{	
	...
	while( WaitForSingleObject(hGlobalThreadWait,50) != WAIT_OBJECT_0 )
	{
        ...
        ///////////////////////////
        // Send ResetConnection //
        //////////////////////////
        
        static CTime Timer = CTime::GetCurrentTime();
        CTime Current = CTime::GetCurrentTime();
        CTimeSpan Lasped;
            
        Lasped  = Current-Timer;
        if ( Lasped.GetSeconds() >= 100)
            {
		...
		switch(Handler.Station.State)
		{
            ...
            case NAT_STATE_AUTHENTICATE:
                ...
                /////////////////////////////////////////////////
                if ( Handler.m_hMyDatabase.CtlUnitACL(Handler.Station,Handler.m_Database) == 0)
                {
                    printf("CtlUnit has an ACL\n");
                    ShowError(Handler,"Wrong ACL Group");
                    Handler.Station.State = NAT_STATE_SETUP;
                    break;
                }
...
void SetupGetAccount( CHandler &Handler)
{
	...
	packet.data8[0] = 1;
	packet.data8[1] = 3;
	packet.lenS = strlen("High Tech DCS");
	packet.df = NAT_DF_8BIT2|NAT_DF_STRING;
	lstrcpy((char *)packet.dataS, "High Tech DCS");
	Size          = Packet2Buffer(&packet, (BYTE *)&Buffer[0]);
	Handler.Send( (LPSTR)&Buffer[0],Size);
